...
fn test_rotate() {
    ...
    let mut vect = vec![
        vec![2, 29, 20, 26, 16, 28],
        vec![12, 27, 9, 25, 13, 21],
        vec![32, 33, 32, 2, 28, 14],
        vec![13, 14, 32, 27, 22, 26],
        vec![33, 1, 20, 7, 21, 7],
        vec![4, 24, 1, 6, 32, 34],
    ];
    Solution::rotate(&mut vect);
    ...
    assert_eq!(
        ...
        vec![
            vec![4, 33, 13, 32, 12, 2],
            vec![24, 1, 14, 33, 27, 29],
            vec![1, 20, 32, 32, 9, 20],
            vec![6, 7, 27, 2, 25, 26],
            vec![32, 21, 22, 28, 13, 16],
            vec![34, 7, 26, 14, 21, 28]
        ]
    );

    let mut vect = vec![
        vec![24, 4, 38, 2, 21, 18, 33, 40],
        vec![24, 37, 25, 62, 37, 15, 35, 36],
        vec![42, 23, 13, 58, 17, 26, 19, 8],
        vec![32, 48, 9, 58, 36, 18, 40, 61],
        vec![23, 16, 0, 46, 35, 34, 23, 60],
        vec![9, 49, 60, 47, 1, 32, 20, 45],
        vec![56, 34, 40, 11, 61, 60, 20, 30],
        vec![45, 30, 25, 18, 49, 3, 16, 10],
    ];
    Solution::rotate(&mut vect);
    println!("transpose vector is {:?}", vect);
    assert_eq!(
        vect,
        vec![
            vec![45, 56, 9, 23, 32, 42, 24, 24],
            vec![30, 34, 49, 16, 48, 23, 37, 4],
            vec![25, 40, 60, 0, 9, 13, 25, 38],
            vec![18, 11, 47, 46, 58, 58, 62, 2],
            vec![49, 61, 1, 35, 36, 17, 37, 21],
            vec![3, 60, 32, 34, 18, 26, 15, 18],
            vec![16, 20, 20, 23, 40, 19, 35, 33],
            vec![10, 30, 45, 60, 61, 8, 36, 40]
        ]
    );
}
